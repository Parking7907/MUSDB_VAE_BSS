...
class VAE(nn.Module):
    ...
    def forward(self, x):
        #print(x.shape) # x.shape 32 X 784
        mu, logvar = self.encode(x.view(-1, self.dimx))
        #print(mu.shape, logvar.shape) # 32 X 40, 32 X 40
        if self.variational is True:
            z = self.reparameterize(mu, logvar)
            recon_x, recons = self.decode(z)
        else:
            recon_x, recons = self.decode(mu)
        #print(z.shape, recon_x.shape, recons.shape) #z = B X 40, recon_x = B X 784, recons = B X 2 X 784
